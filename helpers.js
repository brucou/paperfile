...
const retrieveSpellingErrors = data => {
  ...
  // We will assume it may occur in any place and search for it by traversing
  // the whole XML tree.
  // ASSUMPTION:
  ...
  // - every mispelled word (i.e. one <w:r><w:t>word</w:t></w:r>) is the object
  // of **one** separate tag -- no grouping of words under a single tag.
  ...
  while (traversalStack.length > 0) {
    ...
    // hence no children elements
      ...
      // ASSUMPTION:
      // The next element has a w:r tag
      // The next next element has attribute `w:type=spellEnd`
      const textElement = traversalStack.shift();
      // ASSUMPTION: the text in `w:proofErr` already comes without spaces.
      // Note that the spelling API currently trims the text input it receives
      // itself, but this is undocumented behavior and may change without notice.
